...
impl From<std::io::Error> for CodexErr {
    fn from(e: std::io::Error) -> Self { CodexErr::Io(e) }
}
...
pub async fn process_exec_tool_call(
    ...
    if let Some(stream) = &stdout_stream {
        let begin = ExecCommandBeginEvent {
            ...
            parsed_cmd: params.command.clone(),
        };
        let _ = stream
            .tx_event
            .send(Event { id: stream.sub_id.clone(), msg: EventMsg::ExecCommandBegin(begin) })
            .await;
    }

    let raw_output_result: std::result::Result<RawExecToolCallOutput, CodexErr> =
        exec(params.clone(), sandbox_policy, stdout_stream.clone()).await;
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            // Emit end event if streaming enabled
            if let Some(stream) = &stdout_stream {
                let end = ExecCommandEndEvent {
                    ...
                    duration,
                    formatted_output: format!("exit={} duration_ms={}", exec_output.exit_code, duration.as_millis()),
                };
                let _ = stream
                    .tx_event
                    .send(Event { id: stream.sub_id.clone(), msg: EventMsg::ExecCommandEnd(end) })
                    .await;
            }
